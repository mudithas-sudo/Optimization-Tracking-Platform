...
import type { User, ActivityCategory, AITool, Target, Permission, UserGroup } from "@/generated/prisma/client";

const TABS = ["Users", "Activity Categories", "AI Tools", "Validation Rules", "Targets Config", "Permissions"] as const;
type Tab = (typeof TABS)[number];

const VALIDATION_RULES = [
  { id: "vr-1", name: "Mandatory evidence above 5 hours claimed time saved", appliesTo: "All submissions" },
...
export function AdministrationTabs({
  ...
  tools: (Omit<AITool, "userSatisfaction"> & { userSatisfaction: number })[];
    ...
    <>
      ...
      {tab === "Users" && (
        <Panel title="Users" subtitle={`${users.length} accounts`}>
          <DataTable<User> searchKeys={["name", "employeeId", "jobTitle"]} searchPlaceholder="Search users..." columns={userColumns} rows={users} getRowKey={(r) => r.id} />
        </Panel>
      )}
        ...
        <Panel title="Targets Configuration" subtitle="Edit target values from the Targets page context; shown here for reference">
          <DataTable<(typeof targets)[number]>
            columns={[
              ...
              { key: "targetValue", label: "Value", render: (r) => `${numFmt(r.targetValue, 0)} ${r.unit === "percent" ? "%" : r.unit}` },
